...
export async function streamLogs(projectUid, req, res) {
    let run = true
    ...
    const trailingLogs = await getLastXLogs(projectUid, config.TRAILING_LOGS_BATCH_SIZE)
    ...
    while (run) {
        try {
            const logs = await tailLogs(projectUid, lastLogId)
            if (!logs) continue
            lastLogId = logs[0].id || '$'
