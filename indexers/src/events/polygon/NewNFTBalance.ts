...
export async function onNewNFTBalanceEvent(eventSpec: StringKeyMap): Promise<StringKeyMap | null> {
    ...
    try {
        if (!data.balance || Number(data.balance) === 0) {
            await SharedTables.query(`DELETE FROM polygon.nft_balances WHERE token_address = $1 AND token_id = $2 AND owner_address = $3 AND chain_id = $4`, 
                [
                    data.tokenAddress,
                    data.tokenId,
                    data.ownerAddress,
