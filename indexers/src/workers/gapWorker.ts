...
class GapWorker {
    ...
    async _upsertLatestInteractions(latestInteractions: StringKeyMap[], attempt: number = 0) {
        ...
        for (const latestInteraction of latestInteractions) {
            const existingRecord = existingInteractionsMap[`${latestInteraction.from}:${latestInteraction.to}`]
            if (!existingRecord) {
        ...
        try {
            await SharedTables.manager.transaction(async (tx) => {
                await Promise.all(this._toChunks(writes, this.chunkSize).map(chunk => {
                    return tx.createQueryBuilder()
                        .insert()
    ...
    async _getLatestInteractions(preSavedRecords: StringKeyMap[]): Promise<{ [key: string]: EthLatestInteraction }> {
        let records = []
        try {
