...
const latestInteractions = () => SharedTables.getRepository(EthLatestInteraction)
...
class GapWorker {
    ...
    constructor(from: number, to?: number | null) {
        this.from = from
        this.to = to
        this.cursor = from
        this.groupSize = 1
    ...
    async _upsertLatestInteractions(latestInteractions: StringKeyMap[], attempt: number = 0) {
        if (!latestInteractions.length) return
        const [updateCols, conflictCols] = this.upsertConstraints.latestInteraction
        const existingInteractionsMap = await this._getLatestInteractions(latestInteractions)
        const writes = []

        for (const latestInteraction of latestInteractions) {
            const existingRecord =
                existingInteractionsMap[`${latestInteraction.from}:${latestInteraction.to}`]
            if (!existingRecord) {
                writes.push(latestInteraction)
                continue
            }

            const blockNumber = Number(latestInteraction.blockNumber)
            const existingBlockNumber = Number(existingRecord.blockNumber)
            if (existingBlockNumber >= blockNumber) continue
            writes.push(latestInteraction)
        }
        ...
        try {
            await SharedTables.manager.transaction(async (tx) => {
                await Promise.all(
                    this._toChunks(writes, this.chunkSize).map((chunk) => {
                        return tx
                            .createQueryBuilder()
                            .insert()
            ...
            logger.error(`Error saving latest interactions`, err)
            ...
            if (attempt < 3 && message.toLowerCase().includes('deadlock')) {
                ...
                await sleep(this._randomTimeout(0, 300))
                await this._upsertLatestInteractions(latestInteractions, attempt + 1)
            }
        }
    }

    async _getLatestInteractions(
        preSavedRecords: StringKeyMap[]
    ): Promise<{ [key: string]: EthLatestInteraction }> {
        let records = []
        try {
            records = await latestInteractions().find({
                where: preSavedRecords.map((li) => ({ from: li.from, to: li.to })),
            })
        } catch (err) {
            logger.error(err)
            return {}
        ...
        const latestInteractionsMap = {}
        for (const record of records) {
            latestInteractionsMap[`${record.from}:${record.to}`] = record
        }

        return latestInteractionsMap
    }
    ...
    _randomTimeout(min: number, max: number): number {
        return Math.floor(Math.random() * (max - min)) + min
    }
}

export function getGapWorker(): GapWorker {
    return new GapWorker(config.FROM, config.TO)
}
