...
const logsRepo = () => SharedTables.getRepository(PolygonLog)

class PolygonDecodeLogWorker {
    from: number 
    ...
    async _updateLogs(logs: PolygonLog[]) {
        ...
        try {
            ...
            await client.query(
                `UPDATE polygon.logs SET event_name = ${tempTableName}.event_name, event_args = ${tempTableName}.event_args FROM ${tempTableName} WHERE polygon.logs.log_index = ${tempTableName}.log_index AND polygon.logs.transaction_hash = ${tempTableName}.transaction_hash`
            )
    ...
    _decodeLog(log: PolygonLog, abi: Abi): PolygonLog | null {
    ...
    _tryDecodingLogAsTransfer(log: PolygonLog): PolygonLog | null {
    ...
    async _getEncodedLogsForBlocks(numbers: number[]): Promise<PolygonLog[]> {
        try {
            return (
                (await logsRepo().find({
                    where: {
...
export function getPolygonDecodeLogWorker(): PolygonDecodeLogWorker {
    return new PolygonDecodeLogWorker(config.FROM, config.TO, config.RANGE_GROUP_SIZE)
}